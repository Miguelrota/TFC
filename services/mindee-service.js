...
async function extractData(filePath) {
    try {
        ...
        // Fetch API Key and Model ID dynamically from database
        const db = require('../database/db');
        const credentials = await db.getMindeeCredentials();
        const activeApiKey = credentials.apiKey;
        const activeModelId = credentials.modelId;

        if (!activeApiKey) {
            throw new Error('La API Key de Mindee no está configurada o es inválida. Revisa la configuración de conexión.');
        }

        if (!activeModelId) {
            throw new Error('El Model ID de Mindee no está configurado. Revisa la configuración de conexión.');
        }

        // Initialize dynamic Mindee client
        const dynamicClient = new Client({ apiKey: activeApiKey });
        ...
        const apiResponse = await dynamicClient.enqueueAndGetResult(
            product.Extraction,
            inputSource,
