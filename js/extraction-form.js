...
class ExtractionForm {
    ...
    updateVencimiento(id, field, value) {
        ...
        // Sync back to main field if it's the first installment and date changed
        if (field === 'fecha' && this.vencimientos.indexOf(venc) === 0 && this.invoiceFields.fechaVencimiento) {
            this.invoiceFields.fechaVencimiento.value = value;
        }
    }
    ...
    setupFieldListeners() {
        ...
        Object.entries(this.invoiceFields).forEach(([fieldName, fieldElement]) => {
            ...
            fieldElement.addEventListener('input', (e) => {
                ...
                // Sync Fecha Vencimiento with first installment if changed
                if (fieldName === 'fechaVencimiento' && this.vencimientos.length === 1) {
                    this.vencimientos[0].fecha = e.target.value;
                    this.renderVencimientos();
                }
     ...
     */
    clear() {
        // Clear all Step 1 field values
        Object.values(this.fields).forEach(fieldElement => {
            if (fieldElement) {
                fieldElement.value = '';
                fieldElement.classList.remove('is-invalid', 'is-warning', 'is-valid', 'is-success');
        ...
        // Clear all Step 2 field values
        if (this.invoiceFields) {
            Object.values(this.invoiceFields).forEach(fieldElement => {
                if (fieldElement) {
                    // Don't clear sociedad if it was autocompleted from parent, keep it for convenience
                    // But clear the rest
                    if (fieldElement.id !== 'idSociedad') {
                        fieldElement.value = '';
                    }
                    fieldElement.classList.remove('is-invalid', 'is-warning', 'is-valid', 'is-success');
                }
        ...
        this.lines = [];
        this.vencimientos = [];
        this.setProveedorConfirmado(false);
        this.setStep(1);
        ...
        console.log('Form fully cleared (Step 1 & 2)');
    }
