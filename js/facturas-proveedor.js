(() => {
  ...
  let lastChangeStamp = null;
  let syncTimer = null;
  let syncReloadPromise = null;
  ...
  const els = {
    ...
    btnDeleteInvoice: document.getElementById("btnDeleteInvoice"),
  };
  ...
  function getCurrentSociedadId() {
    const socVal = els.sociedad.value;
    return socVal ? parseInt(socVal) : null;
  }

  function stringifyStamp(stamp) {
    return JSON.stringify(stamp || {});
  }

  async function refreshChangeStamp() {
    try {
      const api = window.api || window.electronAPI;
      if (!api?.getFacturasChangeStamp) return null;

      const sociedadId = getCurrentSociedadId();
      if (!sociedadId) {
        lastChangeStamp = null;
        return null;
      }

      const stamp = await api.getFacturasChangeStamp(sociedadId);
      if (!stamp) return null;
      lastChangeStamp = stringifyStamp(stamp);
      return lastChangeStamp;
    } catch (err) {
      console.warn("Sincronizacion en tiempo real no disponible:", err.message);
      return null;
    }
  }

  async function reloadFromExternalChange(reason = "external") {
    if (syncReloadPromise) return syncReloadPromise;

    syncReloadPromise = (async () => {
      const modal = document.getElementById("invoiceDetailModal");
      const openDetailId = modal?.style.display === "flex" ? modal.dataset.currentInvoiceId : null;

      await loadFacturas();
      await refreshChangeStamp();

      if (openDetailId) {
        await openInvoiceDetail(openDetailId);
      }

      console.log(`Facturas actualizadas por cambio ${reason}.`);
    })().catch((err) => {
      console.error("Error sincronizando facturas:", err);
    }).finally(() => {
      syncReloadPromise = null;
    });

    return syncReloadPromise;
  }

  async function checkForExternalChanges() {
    const api = window.api || window.electronAPI;
    if (!api?.getFacturasChangeStamp) return;

    const sociedadId = getCurrentSociedadId();
    if (!sociedadId) return;

    try {
      const remoteStamp = await api.getFacturasChangeStamp(sociedadId);
      if (!remoteStamp) return;
      const stamp = stringifyStamp(remoteStamp);
      if (lastChangeStamp === null) {
        lastChangeStamp = stamp;
        return;
      }
      if (stamp !== lastChangeStamp) {
        await reloadFromExternalChange("detectado en BD");
      }
    } catch (err) {
      console.warn("No se pudo comprobar la sincronizacion en tiempo real:", err.message);
    }
  }
  ...
  function setupRealtimeSync() {
    ...
    if (api?.onFacturasChanged) {
      api.onFacturasChanged((payload = {}) => {
        const currentSociedadId = getCurrentSociedadId();
        if (!currentSociedadId) return;
        if (payload.sociedadId && Number(payload.sociedadId) !== currentSociedadId) return;
        reloadFromExternalChange("recibido por IPC");
      });
    }

    if (syncTimer) clearInterval(syncTimer);
    syncTimer = setInterval(checkForExternalChanges, 3000);
  ...
  function wireEvents() {
    ...
    document.addEventListener("click", (ev) => {
      ...
      if (row && row.dataset.id) {
          ...
          if (ev.target.closest('button') || ev.target.closest('.lnk')) return;
      ...
      } else if (!ev.target.closest('#btnDeleteInvoice') && !ev.target.closest('.topbar') && !ev.target.closest('.filters')) {
    ...
    function updateDeleteButtonState() {
        if (els.btnDeleteInvoice) {
            const count = selectedInvoiceIds.size;
            els.btnDeleteInvoice.textContent = count > 0 ? `Eliminar (${count})` : "Eliminar Factura";
            els.btnDeleteInvoice.disabled = count === 0;
            els.btnDeleteInvoice.style.opacity = count === 0 ? '0.5' : '1';
        }
    }
    ...
    document.getElementById("btn-add-payment").addEventListener("click", async () => {
        ...
        const rawImporte = document.getElementById("add-pay-importe").value.replace(',', '.');
        const paymentData = {
            fecha: document.getElementById("add-pay-venc").value, // From form
            importe: Math.abs(parseFloat(rawImporte)),
    ...
    if (els.btnDeleteInvoice) {
        els.btnDeleteInvoice.addEventListener("click", () => {
            ...
            const msg = count === 1 
                ...
                : `¿Estás seguro de que deseas eliminar las ${count} facturas seleccionadas permanentemente?`;

            showCustomConfirm(`${msg} Esta acción no se puede deshacer.`, async () => {
                try {
                    const api = window.api || window.electronAPI;
                    
                    // Use a sequential loop instead of Promise.all to avoid deadlocks
                    const idsToDelete = Array.from(selectedInvoiceIds);
                    for (const id of idsToDelete) {
                        await api.deleteInvoice(parseInt(id));
                    }
                    ...
                    console.log(`${count} facturas eliminadas con éxito`);
                } catch (err) {
  ...
  async function openInvoiceDetail(id) {
    try {
      ...
      vencTbody.innerHTML = (vencimientos || []).map((v, idx, arr) => {
        // Calculate incremental amount for display: current cumulative - previous cumulative
        const prevPaid = idx > 0 ? (Number(arr[idx-1].importePagado) || 0) : 0;
        const incremental = (Number(v.importePagado) || 0) - prevPaid;
        
        return `
        <tr>
          <td>${fmtDate(v.fecha)}</td>
          <td class="r">${fmtCurrency(v.importe)}</td>
          <td class="r">${fmtCurrency(incremental)}</td>
      ...
      vencTbody.querySelectorAll("[data-delete-venc]").forEach(btn => {
        btn.addEventListener("click", (e) => {
          ...
          showCustomConfirm("¿Seguro que quieres eliminar este pago?", async () => {
            try {
              const invoiceIdToRefresh = id;
              await api.deleteInvoicePayment(parseInt(vencId));
              
              await openInvoiceDetail(invoiceIdToRefresh);
              await loadFacturas();
            } catch (err) {
      ...
      const totalPagado = (vencimientos && vencimientos.length > 0) 
        ? Math.max(...vencimientos.map(v => Number(v.importePagado) || 0)) 
        : 0;
      ...
      // Set default dates and clear importe
      let defVenc = "";
      try {
        if (header.fechaVencimiento) {
          const dVenc = new Date(header.fechaVencimiento);
          if (!isNaN(dVenc.getTime())) {
            defVenc = dVenc.toISOString().split('T')[0];
          }
        }
      } catch (e) { console.warn("Invalid venc date:", header.fechaVencimiento); }
      ...
      // Focus on amount field to ensure keyboard interaction
      // Enforce window focus and add a simulated click for robustness
      setTimeout(() => {
          const inp = document.getElementById("add-pay-importe");
          if (inp) {
              window.focus(); 
              inp.disabled = false;
              inp.readOnly = false;
              ...
              inp.focus();
              inp.click(); // Some browsers require a click to fully activate input
              
              // Failsafe second attempt
              setTimeout(() => {
          ...
          document.querySelector(".detail-modal")?.style.setProperty("pointer-events", "auto", "important");
      }, 600);
    } catch (err) {
