    ...
    initDragAndDrop() {
        ...
        document.addEventListener('drop', async (e) => {
            ...
            if (files && files.length > 0) {
                const pdfs = files.filter(f => /\.(pdf|jpg|jpeg|png|tiff|tif)$/i.test(f.name));
                if (pdfs.length > 0 && window.bulkProcessor) {
                    window.bulkProcessor.addFilesToQueue(pdfs);
                } else if (pdfs.length > 0) {
                    // Fallback
                    const api = window.electronAPI || window.api || (window.parent && (window.parent.electronAPI || window.parent.api));
                    const filePath = api && api.getPathForFile ? api.getPathForFile(pdfs[0]) : pdfs[0].path;
    ...
    btnLoadDocument.addEventListener('click', async () => {
        try {
            ...
            if (result) {
                ...
                // Always use bulk processor to handle queueing and auto-next
                if (window.bulkProcessor) {
                    const files = filePaths.map(path => ({
                        name: path.split(/[\\/]/).pop(),
                        path: path
                    }));
                    window.bulkProcessor.addFilesToQueue(files);
                } else {
                    // Fallback
                    await invoiceViewer.loadDocument(filePaths[0]);
                }
                console.log('Documentos enviados a cola:', filePaths.length);
            }
        } catch (error) {
            console.error('Error al abrir el explorador:', error);
            if (window.extractionController) {
                window.extractionController.showToast('Error al seleccionar archivos', 'error');
            }
        }
    });
